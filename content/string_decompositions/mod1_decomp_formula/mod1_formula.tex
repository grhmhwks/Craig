...
Fix
\[
  \tau\ge2,\qquad s\ge1,\qquad r=\tau s+1,
...
For $s=1$, the only word is $(0)$ and has defect zero.  For $s=2$, the
words are $(0,a)$, $0\le a\le\tau$, and
\[
 \area(0,a)=a,\qquad \dinv_\tau(0,a)=\tau-a,\qquad
 \defc_\tau(0,a)=0.
\]
On the other hand, $B=-\tau-1$ for $s=1$ and $B=-1$ for $s=2$.
Thus the range $\defc_\tau\le B$ is empty in both cases.

For $s\le2$, define the following three root sets to be empty.  For
$s\ge3$, define
\begin{align*}
 \mathcal F_B
...
We assume the known $q,t$ symmetry of
$\Cat_{\tau s+1,s}(q,t)$.  The required lower-half decompositions are
proved in \texttt{sleq4\_decomp.tex} for $3\le s\le4$, in
\texttt{s5\_decomp.tex} for $s=5$, and in \texttt{sgeq6\_decomp.tex} for
$s\ge6$.  Only the last of these uses the East5 and West5 nonfailure
hypotheses made there.  The small decomposition theorems assume the
full-skeleton position bound, and the general theorem assumes its
special-skeleton version; we prove the stronger required bound here.
...
\]
For the strict lower-half assertion, if $d=\defc_\tau(S)\le B$, then
\[
  2\area(S)+d\le3d\le3B<M.
\]
Indeed,
\[
 M-3B=\tau\frac{(s-3)(s-4)}2+3>0
\]
for every $s\ge3$.
\end{proof}
...
Assume the $q,t$ symmetry and, when $s\ge6$, the East5 and West5 nonfailure
hypotheses used in \texttt{sgeq6\_decomp.tex}.  For every $s\ge1$, the weak
lower half of every defect layer $d\le B$ is partitioned into strings whose
roots may be indexed equivalently by the defect-$d$ subsets of
\[
  \mathcal F_B,
  \qquad \mathcal Z_B,
...
\begin{proof}
If $s=1$ or $s=2$, the inspection in the setup shows that no normalized word
has defect at most $B$.  By convention all three root sets are empty, so
every displayed identity is the equality of empty sums.

Suppose henceforth that $s\ge3$.  Corollary~\ref{cor:skeleton-position}
proves the full-skeleton position hypothesis of
\texttt{sleq4\_decomp.tex} and \texttt{s5\_decomp.tex}.  It also proves the
special-skeleton position hypothesis of \texttt{sgeq6\_decomp.tex}.  For
$s\ge6$, the exceptional full skeleton used there has defect
\[
 \tau(s-2)=B+1,
\]
so its special skeletons in the present range are exactly the full skeletons
in $\mathcal F_B$.

Apply \texttt{sleq4\_decomp.tex} when $3\le s\le4$,
\texttt{s5\_decomp.tex} when $s=5$, and
\texttt{sgeq6\_decomp.tex} when $s\ge6$.  In every case, the weak lower half
of each defect layer is the disjoint union of strings rooted at
$\mathcal F_B$, with one word at every area from $\area(S)$ through the
middle.  The assumed $q,t$ symmetry reflects these strings across the
middle of each defect layer, with a central term counted only once.
Therefore a root $S$ contributes
\[
 \sum_{j=\area(S)}^{\dinv_\tau(S)}
...
\]
Summing the roots gives~\eqref{eq:full-form}.
